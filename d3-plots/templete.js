...
iris.then(function (data) {
  ...
  // This line creates an map of objects which are the grouped data, grouping on the species.
  // The rollup command takes the data, a function, which given a subset of the data aggregates
  // it together to a single object, and a field to group on. This groups by the flower species
  // and returns and object with the quartiles, min, and max as seen in the rollup function above
  // in a map where the key is the species
  const quartilesBySpecies = d3.rollup(data, rollupFunction, (d) => d.Species);

  // This line iterates over each one of the objects in the map, setting the quartiles variable to
  // the object returned from the rollupFunction and the Species variable to the map key which is the
  // species type
